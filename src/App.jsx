...
function HomePage({ setPage, notify }) {
  ...
  const renderChart = (item) => item.id === "trend"
    ...
    : <DashboardPanel title="Распределение оценок" className="home-distribution-panel" onHide={() => hideWidget(item)}><div className="home-distribution-body"><DonutChart /><div className="home-distribution-legend">{[["#b7ff3c", "Отлично (80–100%)", "65%"], ["#39d2c0", "Хорошо (60–80%)", "20%"], ["#7e6bf2", "Удовлетворительно (40–60%)", "10%"], ["#ff7b54", "Плохо (0–40%)", "5%"]].map(([color, label, value]) => <div key={label}><i style={{ background: color }}></i><span>{label}</span><strong>{value}</strong></div>)}</div></div></DashboardPanel>;
  ...
  return <section className="home-dashboard">
    ...
    {catalogOpen && <div className="widget-catalog-layer"><button className="widget-catalog-backdrop" aria-label="Закрыть каталог" onClick={() => setCatalogOpen(false)}></button><section className="widget-catalog" role="dialog" aria-modal="true" aria-labelledby="widget-catalog-title"><header><div><span>Настройка главной</span><h2 id="widget-catalog-title">Каталог виджетов</h2></div><button className="header-icon" aria-label="Закрыть каталог" onClick={() => setCatalogOpen(false)}><IconX size={19} /></button></header><p>Выберите виджет по превью, добавьте его и расставьте блоки на странице перетаскиванием.</p><label className="widget-catalog-search"><IconSearch size={17} /><input value={catalogQuery} onChange={(event) => setCatalogQuery(event.target.value)} placeholder="Найти виджет" autoFocus /></label><div className="widget-catalog-filters">{[["all", "Все"], ["metrics", "Показатели"], ["analytics", "Аналитика"], ["team", "Команда"]].map(([id, label]) => <button className={catalogFilter === id ? "active" : ""} onClick={() => setCatalogFilter(id)} key={id}>{label}</button>)}</div><div className="widget-catalog-list">{shownCatalogItems.map((item) => { const visible = dashboardLayout.includes(item.id); return <article key={item.id}><WidgetCatalogPreview item={item}/><footer><div><strong>{item.label}</strong><small>{item.catalogGroup === "metrics" ? "Показатель" : item.catalogGroup === "analytics" ? "Аналитика" : "Команда"}</small></div><button className={visible ? "visible" : "add"} onClick={() => visible ? hideWidget(item) : addWidget(item)}>{visible ? <><IconCheck size={15} />На главной</> : <><IconPlus size={15} />Добавить</>}</button></footer></article>; })}{shownCatalogItems.length === 0 && <div className="widget-catalog-empty">Ничего не найдено</div>}</div>{hiddenWidgets.length > 0 && <button className="widget-restore-all" onClick={() => { hiddenWidgets.forEach((id) => { const item = catalogItems.find((catalogItem) => catalogItem.id === id); if (item) addWidget(item); }); }}>Добавить все скрытые</button>}</section></div>}
  </section>;
}
...
function Sparkline({ values }) {
  const draw = (context, width, height) => { const padding = 3; const min = Math.min(...values); const max = Math.max(...values); context.beginPath(); values.forEach((value, index) => { const x = padding + (index / (values.length - 1)) * (width - padding * 2); const y = height - padding - ((value - min) / Math.max(1, max - min)) * (height - padding * 2); index ? context.lineTo(x, y) : context.moveTo(x, y); }); context.strokeStyle = "#b7ff3c"; context.lineWidth = 2; context.lineCap = "round"; context.lineJoin = "round"; context.stroke(); };
...
function DashboardLineChart() {
  ...
  const draw = (context, width, height) => {
    ...
    context.font = "10px Inter, Arial, sans-serif"; context.fillStyle = "#77808b"; context.textAlign = "right"; context.textBaseline = "middle";
    [0, 25, 50, 75, 100].forEach((tick) => { const y = top + chartH - (tick / 100) * chartH; context.strokeStyle = "#252d38"; context.lineWidth = 1; context.beginPath(); context.moveTo(left, y); context.lineTo(width - right, y); context.stroke(); context.fillText(`${tick}%`, left - 10, y); });
    const points = values.map((value, index) => [left + (index / (values.length - 1)) * chartW, top + chartH - (value / 100) * chartH]);
    const fill = context.createLinearGradient(0, top, 0, top + chartH); fill.addColorStop(0, "rgba(183,255,60,.24)"); fill.addColorStop(1, "rgba(183,255,60,.01)"); context.beginPath(); context.moveTo(points[0][0], top + chartH); points.forEach(([x, y]) => context.lineTo(x, y)); context.lineTo(points.at(-1)[0], top + chartH); context.closePath(); context.fillStyle = fill; context.fill();
    context.beginPath(); points.forEach(([x, y], index) => index ? context.lineTo(x, y) : context.moveTo(x, y)); context.strokeStyle = "#b7ff3c"; context.lineWidth = 2; context.stroke();
    points.forEach(([x, y]) => { context.beginPath(); context.arc(x, y, 4, 0, Math.PI * 2); context.fillStyle = "#141a23"; context.fill(); context.strokeStyle = "#b7ff3c"; context.lineWidth = 2; context.stroke(); });
    context.textAlign = "center"; context.textBaseline = "top"; context.fillStyle = "#77808b"; labels.forEach((label, index) => label && context.fillText(label, left + (index / (labels.length - 1)) * chartW, height - 18));
  };
...
function DonutChart() {
  const draw = (context, width, height) => { const size = Math.min(width, height); const cx = width / 2, cy = height / 2, radius = size * .37, thickness = size * .12; let start = -Math.PI / 2; [[65, "#b7ff3c"], [20, "#39d2c0"], [10, "#7e6bf2"], [5, "#ff7b54"]].forEach(([value, color]) => { const end = start + (value / 100) * Math.PI * 2; context.beginPath(); context.arc(cx, cy, radius, start, end); context.strokeStyle = color; context.lineWidth = thickness; context.stroke(); start = end; }); context.textAlign = "center"; context.fillStyle = "#f4f7f5"; context.font = "600 22px Inter, Arial, sans-serif"; context.fillText("2 001", cx, cy + 1); context.fillStyle = "#77808b"; context.font = "10px Inter, Arial, sans-serif"; context.fillText("звонков", cx, cy + 21); };
