...
function HomePage({ setPage, notify, balanceCents }) {
  ...
  const renderSummary = (item) => {
    ...
    if (item.id === "employees") return <DashboardPanel title="Топ сотрудники" onHide={() => hideWidget(item)}><div className="home-employee-head"><span>Сотрудник</span><span>Средняя оценка</span><span>Кол-во звонков</span></div><div className="home-employee-list">{employees.map(([avatar, name, score, calls]) => { const scoreValue = Number.parseInt(score, 10); const scoreTone = scoreValue >= 90 ? "high" : scoreValue >= 80 ? "medium" : "low"; return <div key={name}><img src={avatar} alt="" /><strong>{name}</strong><span className={`employee-score ${scoreTone}`}><i></i>{score}</span><span>{calls}</span></div>; })}</div><button className="home-panel-link" onClick={() => { setPage("settings"); notify("Открыт раздел сотрудников"); }}>Все сотрудники</button></DashboardPanel>;
    return <DashboardPanel title="Требует внимания" onHide={() => hideWidget(item)}><div className="home-attention-head"><span>Сотрудник</span><span>Средняя оценка</span><span>Причина</span></div><div className="home-attention-list">{attention.map(([avatar, name, issue, score, tone]) => { const scoreValue = Number.parseInt(score, 10); const scoreTone = scoreValue >= 80 ? "high" : scoreValue >= 50 ? "medium" : "low"; return <div key={name}><img src={avatar} alt="" /><strong>{name}</strong><span className={`employee-score attention-score ${scoreTone}`}><i></i>{score}</span><span className={`attention-reason ${tone}`}>{issue}</span></div>; })}</div><button className="home-panel-link" onClick={() => setPage("analytics")}>Все вопросы</button></DashboardPanel>;
  };
